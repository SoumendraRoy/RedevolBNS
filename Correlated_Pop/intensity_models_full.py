...
with h5py.File("gmm.h5",'r') as hf:
    mean = jnp.array(hf['mean'])
    cov = jnp.array(hf['cov'])
    weight = jnp.array(hf['weight'])
hf.close()
...
def cosmo_model(Mczs, logqs, Thetas, ds, Mczsdetdraw, logqsdetdraw, Thetasdetdraw, dsdetdraw, Ndraw):
    ...
    data_points = jnp.array([Mc.flatten(),logqs.flatten(),z.flatten()]).T
    var3 = weighted_sum_logpdf(data_points, mean, cov, weight).reshape(Nobs,Nsamp)
    data_points_r = jnp.array([Mc.flatten(),logqsr.flatten(),z.flatten()]).T
    var3r = weighted_sum_logpdf(data_points_r, mean, cov, weight).reshape(Nobs,Nsamp)
    var3t = jss.logsumexp(jnp.array([var3, var3r]), axis=0)
    ...
    data_points_sel = jnp.array([Mcdetdraw,logqsdetdraw,zdetdraw]).T
    var3sel = weighted_sum_logpdf(data_points_sel, mean, cov, weight)
    data_points_sel_r = jnp.array([Mcdetdraw,logqsrdetdraw,zdetdraw]).T
    var3rsel = weighted_sum_logpdf(data_points_sel_r, mean, cov, weight)
    var3tsel = jss.logsumexp(jnp.array([var3sel, var3rsel]), axis=0)
    var4sel = jsst.beta.logpdf(Thetasdetdraw, 2, 4)
    var5sel = jss.logsumexp(varsel+var2sel+var3tsel+var4sel)
